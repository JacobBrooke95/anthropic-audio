...
Published: 2026-09-08 · Claude Blog
...
- **Be careful changing effort setting mid-conversation. **These settings render into the prompt ahead of your content, so they are part of the cached prefix. Only with select Claude models, including Opus 5 and Fable 5.1, can you[ update effort mid-conversation](https://platform.claude.com/docs/en/build-with-claude/effort#changing-effort-mid-conversation) without breaking the cache.
...
- **Move the cache breakpoint as the conversation grows**. With Claude Platform, you can set [automatic caching](https://platform.claude.com/docs/en/build-with-claude/prompt-caching#automatic-caching) to apply the cache breakpoint to the last cacheable block. 
